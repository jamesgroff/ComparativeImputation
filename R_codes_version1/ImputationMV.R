...
# Those average CM should be made as figures, save them for making figures in VisualizeMetrics.R
...
filter_rows <- function(original_matrices, significant_dataframe){
  ...
  for (i in (1:length(original_matrices))){
    ...
    # Store the original data in the current category
    result_list[[category_name]] <- as.data.frame(significant_dataframe[row_names,column_index], row.names = as.integer(row_names))
  }
