...
# There are lapply warnings during the identification of DE proteins/phosphosites (in gaussian and mle DE identification), fix it somehow
# REML=FALSE when calling the linear model to fix problems (Rohr's advice in mail)
# Make standard variations or deviations of the metrics and on the plots (you could reduce the code...)
...
# Additionally, make sure that you remove all the rows that has missing values from the generated matrices and do all the imputation processes
...
# Remove all the full NA rows from those generated matrices
has_non_NA <- function(single_row){
  any(!is.na(single_row))
}

remove_full_row_NA<- function(matrix_list){
  ...
  for (i in 1:length(matrix_list)){
    ...
    for (j in 1:length(matrix_list[[i]])){
      ...
      # Get indexes of rows with at least one complete value
      complete_indexes <- which(apply(single_matrix, 1, has_non_NA))
...
missing_complete_matrices_prot <- remove_full_row_NA(missing_matrices_prot)
missing_complete_matrices_phos <- remove_full_row_NA(missing_matrices_phos)
missing_complete_matrices_norm <- remove_full_row_NA(missing_matrices_norm)
...
knn_imputation_prot <- function(matrix_list){
  for (i in 1:length(matrix_list)){
    for (j in 1:length(matrix_list[[i]])){
      ...
      imputed_matrix <- impute.knn(data_numeric, k=ideal_k, rowmax=0.8)
...
knn_imputation_phos <- function(matrix_list){
  for (i in 1:length(matrix_list)){
    for (j in 1:length(matrix_list[[i]])){
      ...
      imputed_matrix <- impute.knn(data_numeric, k=ideal_k, maxp=5000, rowmax=0.8)
      complete_matrix <- imputed_matrix$data
      # If needed, convert back to dataframe
      matrix_list[[i]][[j]] <- as.data.frame(complete_matrix, row.names=row_indexes)
    }
...
knn_imputed_matrices_prot <- knn_imputation_prot(missing_complete_matrices_prot)
knn_imputed_matrices_phos <- knn_imputation_phos(missing_complete_matrices_phos)
knn_imputed_matrices_norm <- knn_imputation_phos(missing_complete_matrices_norm)
  ...
  for (i in 1:length(imputed_significant_list)){
    ...
    for (j in 1:length(imputed_significant_list[[i]])){
      ...
      if (ncol(imputed_data)!=0){
        ...
        confusion_matrix <- confusionMatrix(data=factored_original_data, reference=factored_imputed_data)
...
average_CM_overall_proteinGroups <- list(minmax=average_CM_min_max_prot, gaussian=average_CM_gaussian_prot, knn=average_CM_knn_prot, mle=average_CM_mle_prot, rf=average_CM_rf_prot)
average_CM_overall_Phophosites <- list(minmax=average_CM_min_max_phos, gaussian=average_CM_gaussian_phos, knn=average_CM_knn_phos, mle=average_CM_mle_phos, rf=average_CM_rf_phos)
average_CM_overall_normedPhosphosites <- list(minmax=average_CM_min_max_norm, gaussian=average_CM_gaussian_norm, knn=average_CM_knn_norm, mle=average_CM_mle_norm, rf=average_CM_rf_norm)
...
calculate_metrics <- function(average_cm_list, cm_list, imputation_type){
  ...
  for (i in 1:length(average_cm_list)){
    ...
    if(all(is.na(conf_matrix))){
      metrics <- data.frame(FDR = NA, FDR_sd = NA, Accuracy = NA, Accuracy_sd = NA, MCC = NA, MCC_sd = NA)
    ...
    metrics_table <- data.frame(matrix(ncol=3, nrow=20, dimnames=list(NULL, c("FDR","Accuracy","MCC"))))
    ...
    for (j in 1:length(cm_list[[i]])){
      ...
      metrics_table[j,] <- c(FDR_val, accuracy_val, MCC_val)
    }
...
metrics_CM_proteinGroups <- base::rbind(min_max_metrics_prot, gaussian_metrics_prot, knn_metrics_prot, mle_metrics_prot, rf_metrics_prot)
metrics_CM_Phosphosites <- base::rbind(min_max_metrics_phos, gaussian_metrics_phos, knn_metrics_phos, mle_metrics_phos, rf_metrics_phos)
metrics_CM_normedPhosphosites <- base::rbind(min_max_metrics_norm, gaussian_metrics_norm, knn_metrics_norm, mle_metrics_norm, rf_metrics_norm)
...
avg_nrmse_prot <- base::rbind(avg_nrmse_minmax_prot, avg_nrmse_gaussian_prot, avg_nrmse_knn_prot, avg_nrmse_mle_prot, avg_nrmse_rf_prot)
avg_nrmse_phos <- base::rbind(avg_nrmse_minmax_phos, avg_nrmse_gaussian_phos, avg_nrmse_knn_phos, avg_nrmse_mle_phos, avg_nrmse_rf_phos)
avg_nrmse_norm <- base::rbind(avg_nrmse_minmax_norm, avg_nrmse_gaussian_norm, avg_nrmse_knn_norm, avg_nrmse_mle_norm, avg_nrmse_rf_norm)
