# Notes: Use lme4 package for your default run (if it's really not working, use the nlme package instead)
...
# Additionally, apply a fold change filter as an option

# Required packages to install: stringr, lme4, pracma, dplyr
...
# What sort of statistical tests do you want to apply for your significant DE data? (both results will be combined if both are selected)
mlm_test <- TRUE
sigA_test <- TRUE

# Do you want to apply a fold change filter and get another file with the corresponding fold change?
make_fold_change <- TRUE
# If so, indicate the fold change number below
...
if (mlm_test){print(list_significant_mlm_conditions_proteinGroups)}
...
# Print one of the lists for verification
if (sigA_test){print(list_significant_sigA_conditions_proteinGroups)}
...
# Apply the comparison to all significant lists that are comparable and depending on the choice of significant tests
if (mlm_test & sigA_test){
  if (get_proteinGroups){
  ...
  if (get_proteinGroups){
    full_significant_list_conditions_proteinGroups <- list_significant_mlm_conditions_proteinGroups
  }
  if (get_Phosphosites){
    full_significant_list_conditions_Phosphosites <- list_significant_mlm_conditions_Phosphosites
  }
  if (get_normedPhosphosites){
    full_significant_list_conditions_normedPhosphosites <- list_significant_mlm_conditions_normedPhosphosites
  }
} else if (sigA_test){
  if (get_proteinGroups){
    full_significant_list_conditions_proteinGroups <- list_significant_sigA_conditions_proteinGroups
  }
  if (get_Phosphosites){
    full_significant_list_conditions_Phosphosites <- list_significant_sigA_conditions_Phosphosites
  }
  if (get_normedPhosphosites){
    full_significant_list_conditions_normedPhosphosites <- list_significant_sigA_conditions_normedPhosphosites
  }
}

# Print one of the lists for verification
if (mlm_test | sigA_test){print(full_significant_list_conditions_proteinGroups)}
...
# For instance, apply the resulting significant dataframes with all applicable data and save them in txt formats
if (get_proteinGroups){
...
if (make_fold_change){
  ...
  filter_significant_df <- function(original_df, significant_list){
    # Extract all row IDs from the list
    all_row_ids <- unlist(significant_list)
    ...
    for (category in names(significant_list)){
      # Get all the row ids from the current category
      row_ids <- significant_list[[category]]
      # Get all the columns that apply to this category in the original dataframe
      categories <- colnames(original_df)[grep(category, colnames(original_df))]
      for (row_id in all_row_ids){
        character_id <- as.character(row_id)
        if(!(as.character(row_id) %in% row_ids)){
          # Set values not in the row_ids to NA for this category
          filtered_df[rownames(filtered_df) == character_id, colnames(original_df) %in% categories] <- NA
        }
      }
    }
  ...
  if (get_proteinGroups){
    filtered_conditions_proteinGroups <- filter_significant_df(conditions_by_label_proteinGroups, full_significant_list_conditions_proteinGroups)
  }
  if (get_Phosphosites){
    filtered_conditions_Phosphosites <- filter_significant_df(conditions_by_label_Phosphosites, full_significant_list_conditions_Phosphosites)
  }
  if (get_normedPhosphosites){
    filtered_conditions_normedPhosphosites <- filter_significant_df(conditions_by_label_normedPhosphosites, full_significant_list_conditions_normedPhosphosites)
  }
  
  # Make fold change filter for these filtered conditions data (1.5x)
  fold_change_filter <- function(df, fold_change){
  ...
  if (get_proteinGroups){
    fold_changed_conditions_proteinGroups <- fold_change_filter(filtered_conditions_proteinGroups, fold_change_threshold)
  }
  if (get_Phosphosites){
    fold_changed_conditions_Phosphosites <- fold_change_filter(filtered_conditions_Phosphosites, fold_change_threshold)
  }
  if (get_normedPhosphosites){
    fold_changed_conditions_normedPhosphosites <- fold_change_filter(filtered_conditions_normedPhosphosites, fold_change_threshold)
  }
  ...
  if (get_proteinGroups){
    list_fold_significant_conditions_proteinGroups <- fold_df_to_list(fold_changed_conditions_proteinGroups)
  }
  if (get_Phosphosites){
    list_fold_significant_conditions_Phosphosites <- fold_df_to_list(fold_changed_conditions_Phosphosites)
  }
  if (get_normedPhosphosites){
    list_fold_significant_conditions_normedPhosphosites <- fold_df_to_list(fold_changed_conditions_normedPhosphosites)
  }
  
  # Print one of the lists for verification
  if (get_proteinGroups){print(list_fold_significant_conditions_proteinGroups)}
  
  # Add columns to the original data to signify if the proteins/peptides are significantly DE 
  # and have a fold change mean more than 1.5 depending on the conditions/label with a "+"
  
  if (get_proteinGroups){
    significant_fold_conditions_proteinGroups <- add_id_columns(proteinGroups, list_fold_significant_conditions_proteinGroups)
  }
  if (get_Phosphosites){
    significant_fold_conditions_Phosphosites <- add_id_columns(Phosphosites, list_fold_significant_conditions_Phosphosites)
  }
  if (get_normedPhosphosites){
    significant_fold_conditions_normedPhosphosites <- add_id_columns(normedPhosphosites, list_fold_significant_conditions_normedPhosphosites)
  }
  
  # Save the significant DE proteins/phosphosites with fold change filter in txt format
  if (get_proteinGroups){
    write.table(significant_fold_conditions_proteinGroups, file="proteinGroups_significant_with_fold_change.txt", sep="\t", row.names=FALSE)
  }
  if (get_Phosphosites){
    write.table(significant_fold_conditions_Phosphosites, file="Phospho (STY)Sites_significant_with_fold_change.txt", sep="\t", row.names=FALSE)
  }
  if (get_normedPhosphosites){
    write.table(significant_fold_conditions_normedPhosphosites, file="Phospho (STY)Sites_normed_significant_with_fold_change.txt", sep="\t", row.names=FALSE)
  }
}
