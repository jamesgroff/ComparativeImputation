# Notes: Check if imputations are random values or not. (minmax, gaussian and mle will be random, knn and rf will have same values)
...
# There is a problem with IB_01 generated missing data in phosphosites with Exp1 having abnormal high values (origin: 13095000 at row 5590 from original phos data)
...
# Then make confusion matrices and average them out for method comparisons with metrics such as FDR, accuracy and Matthews correlation coefficient (NRMSE too as an option)
# Additionally, make sure that you remove all the rows that has no or 1 full values from the generated matrices and do all the imputation processes
# This version also makes sure that the fold change filter has been set for comparisons of with and without its application

# Required packages to install: stringr, mice, impute, imp4p, missForest, lme4, pracma, dplyr, caret
...
# Get all the original datasets of proteins and phosphosites (STY and normed) and their respective significant dataframes (with or without fold changes)
...
significant_fold_proteinGroups <- read.table("proteinGroups_significant_with_fold_change.txt", header=TRUE, sep="\t")
significant_fold_Phosphosites <- read.table("Phospho (STY)Sites_significant_with_fold_change.txt", header=TRUE, sep="\t")
significant_fold_normedPhosphosites <- read.table("Phospho (STY)Sites_normed_significant_with_fold_change.txt", header=TRUE, sep="\t")
...
# We are only going to be interested into the condition label ratios for this project, so pick only those ones as significance basis for the normed Phosphosites
...
condition_significant_fold_normedPhosphosites <- significant_fold_normedPhosphosites[,desired_significant_columns]
...
condition_significant_fold_proteinGroups <- significant_fold_proteinGroups[rownames(proteinGroups),]
condition_significant_fold_Phosphosites <- significant_fold_Phosphosites[rownames(Phosphosites),]
...
colnames(condition_significant_fold_proteinGroups) <- substitute_colnames(desired_significant_columns)
colnames(condition_significant_fold_Phosphosites) <- substitute_colnames(desired_significant_columns)
colnames(condition_significant_fold_normedPhosphosites) <- substitute_colnames(desired_significant_columns)
...
filtered_significant_fold_prot <- filter_rows(original_matrices_prot, condition_significant_fold_proteinGroups)
filtered_significant_fold_phos <- filter_rows(original_matrices_phos, condition_significant_fold_Phosphosites)
filtered_significant_fold_norm <- filter_rows(original_matrices_norm, condition_significant_fold_normedPhosphosites)

# Get all the generated artificial missing values from the different folders and store them into a list
...
# Function to check if the number of non-NA values in a row is greater than 1
has_more_than_one_non_NA <- function(single_row){
  sum(!is.na(single_row)) > 1
}

# Remove all the full NA or one complete value only from the rows of those generated matrices
remove_full_and_near_row_NA<- function(matrix_list){
  # Initialize a list to store all the results
  complete_matrix_list <- list()
  
  for (i in 1:length(matrix_list)){
    # Get the category name of the missing matrix list
    original_name <- names(matrix_list)[i]
    
    for (j in 1:length(matrix_list[[i]])){
      # Get the single matrix from the list indexes
      single_matrix <- matrix_list[[i]][[j]]
      
      # Get indexes of rows with at least two complete values
      complete_indexes <- which(apply(single_matrix, 1, has_more_than_one_non_NA))
      
      # Subset the original matrix to include only complete cases
      complete_matrix <- single_matrix[complete_indexes,]
      
      # Add this complete matrix into the complete list
      complete_matrix_list[[original_name]][[j]] <- complete_matrix
    }
  }
  
  return(complete_matrix_list)
}

missing_complete_matrices_prot <- remove_full_and_near_row_NA(missing_matrices_prot)
missing_complete_matrices_phos <- remove_full_and_near_row_NA(missing_matrices_phos)
missing_complete_matrices_norm <- remove_full_and_near_row_NA(missing_matrices_norm)

# Make an example matrix to test for one matrix imputation only
test_missing_matrix <- missing_complete_matrices_prot[[1]][[1]]
...
min_max_imputed_matrices_prot <- random_min_max(missing_complete_matrices_prot)
min_max_imputed_matrices_phos <- random_min_max(missing_complete_matrices_phos)
min_max_imputed_matrices_norm <- random_min_max(missing_complete_matrices_norm)
...
gaussian_imputed_matrices_prot <- gaussian_sampling(missing_complete_matrices_prot)
gaussian_imputed_matrices_phos <- gaussian_sampling(missing_complete_matrices_phos)
gaussian_imputed_matrices_norm <- gaussian_sampling(missing_complete_matrices_norm)
...
# Implement knn imputation as such
knn_imputation <- function(matrix_list){
  for (i in 1:length(matrix_list)){
    for (j in 1:length(matrix_list[[i]])){
      current_matrix <- matrix_list[[i]][[j]]
      ...
      imputed_matrix <- impute.knn(data_numeric, k=ideal_k, rowmax=0.6)
...
knn_imputed_matrices_prot <- knn_imputation(missing_complete_matrices_prot)
knn_imputed_matrices_phos <- knn_imputation(missing_complete_matrices_phos)
knn_imputed_matrices_norm <- knn_imputation(missing_complete_matrices_norm)
...
mle_imputed_matrices_prot <- mle_imputation(missing_complete_matrices_prot)
mle_imputed_matrices_phos <- mle_imputation(missing_complete_matrices_phos)
mle_imputed_matrices_norm <- mle_imputation(missing_complete_matrices_norm)
...
rf_imputed_matrices_prot <- rf_imputation(missing_complete_matrices_prot)
rf_imputed_matrices_phos <- rf_imputation(missing_complete_matrices_phos)
rf_imputed_matrices_norm <- rf_imputation(missing_complete_matrices_norm)

# Identify DE proteins/phosphosites from those imputed matrices
# Essentially a copy from BenchmarkStatistics.R, from the mlm implementation to the addition of significant ids, modify them to work for matrix lists
...
compare_lists_ids <- function(combined_list){
  ...
  common_names <- base::intersect(names(combined_list$mlm), names(combined_list$sigA))
  ...
  for (name in common_names){
    ...
    common_ids <- base::intersect(set1, set2)
...
add_id_columns <- function(original_df, id_list, imputed_matrix){
  # Update the original dataframe so that it has the same rows as the imputed matrix
  original_matrix <- original_df[rownames(original_df) %in% rownames(imputed_matrix),]
  
  # Create a matrix to store the values with original row names from the original dataframe
  significant_matrix <- matrix(NA, nrow=nrow(original_matrix), ncol=length(id_list))
  row.names(significant_matrix) <- row.names(original_matrix)
  
  # Iterate through the list of IDs
  for (i in seq_along(id_list)){
    id <- names(id_list)[i]
    significant_matrix[,i] <- ifelse(rownames(original_matrix) %in% id_list[[id]], "+", "")
  }
...
call_significant <- function(matrix_list, original_list){
  ...
  for (i in 1:length(matrix_list)){
    ...
    for (j in 1:length(matrix_list[[i]])){
      ...
      matrix_list[[i]][[j]] <- add_id_columns(original_matrix, compared_list, current_matrix)
...
significant_without_prot <- call_significant(missing_complete_matrices_prot, original_matrices_prot)
significant_without_phos <- call_significant(missing_complete_matrices_phos, original_matrices_phos)
significant_without_norm <- call_significant(missing_complete_matrices_norm, original_matrices_norm)
...
# Fold change filter functions
library(dplyr)

# Filter the significant data with respect to the conditions dataframe and the significant list
filter_significant_df <- function(original_df, significant_list){
  # Extract the IDs from the significant list only when the "+" sign is here
  all_row_ids <- rownames(significant_list)[significant_list=="+"]
  
  # Filter the dataframe using the extracted row IDs
  filtered_df <- original_df[rownames(original_df) %in% as.character(all_row_ids),]
  
  return(filtered_df)
}

# Make fold change filter for these filtered conditions data (1.5x)
fold_change_filter <- function(df){
  # Define the conditions separation from the dataframe itself
  condition_names <- unique(gsub("\\d+$","", colnames(df)))
  conditions <- lapply(condition_names, function(cond) grep(cond, names(df), value=TRUE))
  names(conditions) <- condition_names
  
  # Calculate the mean of replicates for each condition
  mean_fold_change <- sapply(conditions, function(cols) rowMeans(df[,cols], na.rm=TRUE))
  
  # If there is only 1 value to be assigned, make sure that the mean fold change is readable (weird column name from previous variable)
  if (length(class(mean_fold_change)) == 1){
    # Make the numeric format into a matrix
    mean_fold_change <- as.data.frame(mean_fold_change)
    
    # Extract the row number after the last dot
    rownames(mean_fold_change) <- sub(".*\\.(\\d+)$", "\\1", rownames(mean_fold_change))
    colnames(mean_fold_change) <- condition_names
  }
  
  # Replace all NaN values into 0 (they will not get passed in the threshold)
  mean_fold_change[is.na(mean_fold_change)] <- 0
  
  # Set the threshold of fold change
  threshold <- 1.5
  
  # Initiate the filtered dataframe being the input dataframe
  filtered_df <- df
  
  # Replace the values that do not pass the threshold with NA
  for (category in condition_names){
    # Get all the columns that apply to this category in the original dataframe
    categories <- colnames(df)[grep(category, colnames(df))]
    
    # Create a logical vector that tests if the mean fold change from the current category is lower than the threshold
    low_threshold <- mean_fold_change[,category] < threshold
    
    # Filter the dataframe based on the mean fold change by setting non-matches as NA
    filtered_df[low_threshold, colnames(filtered_df) %in% categories] <- NA
  }
  
  # Remove rows where all values are NA from the filtered data
  clean_df <- filtered_df[!apply(is.na(filtered_df), 1, all),]
  
  return(clean_df)
}

# Next step would be to indicate the significance from those filtered significant data as "+" or "" like before

fold_df_to_list <- function(fold_df){
  # Create a list to store the results for each category
  result_list <- list()
  
  # Get the unique categories from the fold change dataset
  categories <- unique(gsub("\\d+$","", colnames(fold_df)))
  
  # Iterate through the different categories
  for (category in categories){
    # Subset dataframe for the current category
    cat_cols <- grep(paste0("^", category), colnames(fold_df))
    cat_df <- fold_df[, cat_cols, drop = FALSE]
    
    # Remove all rows that have at least one missing value to only get complete matrices
    cat_df <- na.omit(cat_df)
    
    # Determine the indexes by extracting its row names
    row_indexes <- as.integer(rownames(cat_df))
    
    # Store those ids for the current category
    result_list[[category]] <- row_indexes
  }
  
  return(result_list)
}

# Function that applies the fold change filter to those significant lists
call_significant_fold <- function(matrix_list, original_list, significant_list){
  # Iterate over the matrix list
  for (i in 1:length(matrix_list)){
    # Get the original matrix from the category of choice
    original_matrix <- original_list[[i]]
    for (j in 1:length(matrix_list[[i]])){
      # Get the current matrix to be filtered with its associated significant list
      current_matrix <- matrix_list[[i]][[j]]
      current_significant_list <- significant_list[[i]][[j]]
      
      # If there are no columns in the significant list, return the current list itself
      if(ncol(current_significant_list) == 0){
        matrix_list[[i]][[j]] <- current_significant_list
        next
      }
      
      # Call the significant data filter function with the current matrix and its significant list as arguments
      filtered_conditions <- filter_significant_df(current_matrix, current_significant_list)
      
      # Call the fold change filter function
      fold_changed_conditions <- fold_change_filter(filtered_conditions)
      
      # Indicate significance from the fold changed data as a list
      list_fold_significant <- fold_df_to_list(fold_changed_conditions)
      
      # Add "+" to those significant ids
      matrix_list[[i]][[j]] <- add_id_columns(original_matrix, list_fold_significant, current_matrix)
    }
  }
  return(matrix_list)
}

# Eventual calls to identify DE proteins/phosphosites with fold change filter without any imputation methods
significant_fold_without_prot <- call_significant_fold(missing_complete_matrices_prot, original_matrices_prot, significant_without_prot)
significant_fold_without_phos <- call_significant_fold(missing_complete_matrices_phos, original_matrices_phos, significant_without_phos)
significant_fold_without_norm <- call_significant_fold(missing_complete_matrices_norm, original_matrices_norm, significant_without_norm)

# Eventual calls to identify DE proteins/phosphosites with fold change filter for min_max imputation 
significant_fold_min_max_prot <- call_significant_fold(min_max_imputed_matrices_prot, original_matrices_prot, significant_min_max_prot)
significant_fold_min_max_phos <- call_significant_fold(min_max_imputed_matrices_phos, original_matrices_phos, significant_min_max_phos)
significant_fold_min_max_norm <- call_significant_fold(min_max_imputed_matrices_norm, original_matrices_norm, significant_min_max_norm)

# Eventual calls to identify DE proteins/phosphosites for gaussian imputation (better than min_max for negative control)
significant_fold_gaussian_prot <- call_significant_fold(gaussian_imputed_matrices_prot, original_matrices_prot, significant_gaussian_prot)
significant_fold_gaussian_phos <- call_significant_fold(gaussian_imputed_matrices_phos, original_matrices_phos, significant_gaussian_phos)
significant_fold_gaussian_norm <- call_significant_fold(gaussian_imputed_matrices_norm, original_matrices_norm, significant_gaussian_norm)

# Eventual calls to identify DE proteins/phosphosites for knn imputation
significant_fold_knn_prot <- call_significant_fold(knn_imputed_matrices_prot, original_matrices_prot, significant_knn_prot)
significant_fold_knn_phos <- call_significant_fold(knn_imputed_matrices_phos, original_matrices_phos, significant_knn_phos)
significant_fold_knn_norm <- call_significant_fold(knn_imputed_matrices_norm, original_matrices_norm, significant_knn_norm)

# Eventual calls to identify DE proteins/phosphosites for mle imputation
significant_fold_mle_prot <- call_significant_fold(mle_imputed_matrices_prot, original_matrices_prot, significant_mle_prot)
significant_fold_mle_phos <- call_significant_fold(mle_imputed_matrices_phos, original_matrices_phos, significant_mle_phos)
significant_fold_mle_norm <- call_significant_fold(mle_imputed_matrices_norm, original_matrices_norm, significant_mle_norm)

# Eventual calls to identify DE proteins/phosphosites for rf imputation
significant_fold_rf_prot <- call_significant_fold(rf_imputed_matrices_prot, original_matrices_prot, significant_rf_prot)
significant_fold_rf_phos <- call_significant_fold(rf_imputed_matrices_phos, original_matrices_phos, significant_rf_phos)
significant_fold_rf_norm <- call_significant_fold(rf_imputed_matrices_norm, original_matrices_norm, significant_rf_norm)
...
make_confusion_matrices <- function(original_significant_list, imputed_significant_list){
  ...
  for (i in 1:length(imputed_significant_list)){
    ...
    for (j in 1:length(imputed_significant_list[[i]])){
      ...
      # Update the original data vector by only having the same rows as the imputed data
      updated_original_data <- original_data[rownames(original_data) %in% rownames(imputed_data),]
      # Make the original dataframe a factor data (using lapply which is counter-intuitive)
      factored_original_data <- unlist(lapply(updated_original_data, factor))
      ...
      if (ncol(imputed_data)!=0){
        ...
        # Initialize the confusion matrix output
        confusion_matrix <- NULL
        
        # Make a tryCatch statement to capture warnings that can happen when calling the confusionMatrix function
        result <- tryCatch(
          {
        ...
        # Use the result of tryCatch in an if statement to see if we have to ignore some confusion matrices output
        if(result == "Success"){
          result_list[[original_name]][[j]] <- confusion_matrix$table
        } else if(result == "Warning catched"){
          print(paste0("Warning catched in (",i,",",j,")" ))
          result_list[[original_name]][[j]] <- confusion_matrix
        } else {
          result_list[[original_name]][[j]] <- NA
        }
        
      }else{
        # If the column of the imputed data is missing, assign it to "NA" rather than "NULL"
        result_list[[original_name]][[j]] <- NA
      }
    }
  }
...
# Small note: there are warnings that only happen in minmax because of its negative control nature, but for normed data, why?

# Apply the confusion matrices calculation to every applicable data
...
# Also apply it to fold changes significant data
confusion_matrices_fold_without_prot <- make_confusion_matrices(filtered_significant_fold_prot, significant_fold_without_prot)
confusion_matrices_fold_without_phos <- make_confusion_matrices(filtered_significant_fold_phos, significant_fold_without_phos)
confusion_matrices_fold_without_norm <- make_confusion_matrices(filtered_significant_fold_norm, significant_fold_without_norm)

confusion_matrices_fold_min_max_prot <- make_confusion_matrices(filtered_significant_fold_prot, significant_fold_min_max_prot)
confusion_matrices_fold_min_max_phos <- make_confusion_matrices(filtered_significant_fold_phos, significant_fold_min_max_phos)
confusion_matrices_fold_min_max_norm <- make_confusion_matrices(filtered_significant_fold_norm, significant_fold_min_max_norm)

confusion_matrices_fold_gaussian_prot <- make_confusion_matrices(filtered_significant_fold_prot, significant_fold_gaussian_prot)
confusion_matrices_fold_gaussian_phos <- make_confusion_matrices(filtered_significant_fold_phos, significant_fold_gaussian_phos)
confusion_matrices_fold_gaussian_norm <- make_confusion_matrices(filtered_significant_fold_norm, significant_fold_gaussian_norm)

confusion_matrices_fold_knn_prot <- make_confusion_matrices(filtered_significant_fold_prot, significant_fold_knn_prot)
confusion_matrices_fold_knn_phos <- make_confusion_matrices(filtered_significant_fold_phos, significant_fold_knn_phos)
confusion_matrices_fold_knn_norm <- make_confusion_matrices(filtered_significant_fold_norm, significant_fold_knn_norm)

confusion_matrices_fold_mle_prot <- make_confusion_matrices(filtered_significant_fold_prot, significant_fold_mle_prot)
confusion_matrices_fold_mle_phos <- make_confusion_matrices(filtered_significant_fold_phos, significant_fold_mle_phos)
confusion_matrices_fold_mle_norm <- make_confusion_matrices(filtered_significant_fold_norm, significant_fold_mle_norm)

confusion_matrices_fold_rf_prot <- make_confusion_matrices(filtered_significant_fold_prot, significant_fold_rf_prot)
confusion_matrices_fold_rf_phos <- make_confusion_matrices(filtered_significant_fold_phos, significant_fold_rf_phos)
confusion_matrices_fold_rf_norm <- make_confusion_matrices(filtered_significant_fold_norm, significant_fold_rf_norm)
...
# Average out first without fold changed data
...
# Then average out with the fold changed data
average_CM_fold_without_prot <- average_CM(confusion_matrices_fold_without_prot)
average_CM_fold_without_phos <- average_CM(confusion_matrices_fold_without_phos)
average_CM_fold_without_norm <- average_CM(confusion_matrices_fold_without_norm)

average_CM_fold_min_max_prot <- average_CM(confusion_matrices_fold_min_max_prot)
average_CM_fold_min_max_phos <- average_CM(confusion_matrices_fold_min_max_phos)
average_CM_fold_min_max_norm <- average_CM(confusion_matrices_fold_min_max_norm)

average_CM_fold_gaussian_prot <- average_CM(confusion_matrices_fold_gaussian_prot)
average_CM_fold_gaussian_phos <- average_CM(confusion_matrices_fold_gaussian_phos)
average_CM_fold_gaussian_norm <- average_CM(confusion_matrices_fold_gaussian_norm)

average_CM_fold_knn_prot <- average_CM(confusion_matrices_fold_knn_prot)
average_CM_fold_knn_phos <- average_CM(confusion_matrices_fold_knn_phos)
average_CM_fold_knn_norm <- average_CM(confusion_matrices_fold_knn_norm)

average_CM_fold_mle_prot <- average_CM(confusion_matrices_fold_mle_prot)
average_CM_fold_mle_phos <- average_CM(confusion_matrices_fold_mle_phos)
average_CM_fold_mle_norm <- average_CM(confusion_matrices_fold_mle_norm)

average_CM_fold_rf_prot <- average_CM(confusion_matrices_fold_rf_prot)
average_CM_fold_rf_phos <- average_CM(confusion_matrices_fold_rf_phos)
average_CM_fold_rf_norm <- average_CM(confusion_matrices_fold_rf_norm)
...
average_CM_fold_overall_proteinGroups <- list(no=average_CM_fold_without_prot, minmax=average_CM_fold_min_max_prot, gaussian=average_CM_fold_gaussian_prot, knn=average_CM_fold_knn_prot, mle=average_CM_fold_mle_prot, rf=average_CM_fold_rf_prot)
average_CM_fold_overall_Phophosites <- list(no=average_CM_fold_without_phos, minmax=average_CM_fold_min_max_phos, gaussian=average_CM_fold_gaussian_phos, knn=average_CM_fold_knn_phos, mle=average_CM_fold_mle_phos, rf=average_CM_fold_rf_phos)
average_CM_fold_overall_normedPhosphosites <- list(no=average_CM_fold_without_norm, minmax=average_CM_fold_min_max_norm, gaussian=average_CM_fold_gaussian_norm, knn=average_CM_fold_knn_norm, mle=average_CM_fold_mle_norm, rf=average_CM_fold_rf_norm)

# Save those average confusion matrices in RData format for visualization later
save(average_CM_overall_proteinGroups, file="average_CM_proteinGroups_more_than_one_complete_value.RData")
save(average_CM_overall_Phophosites, file="average_CM_Phospho(STY)Sites_more_than_one_complete_value.RData")
save(average_CM_overall_normedPhosphosites, file="average_CM_Phospho(STY)Sites_normed_more_than_one_complete_value.RData")

save(average_CM_fold_overall_proteinGroups, file="average_CM_proteinGroups_with_fold_change.RData")
save(average_CM_fold_overall_Phophosites, file="average_CM_Phospho(STY)Sites_with_fold_change.RData")
save(average_CM_fold_overall_normedPhosphosites, file="average_CM_Phospho(STY)Sites_normed_with_fold_change.RData")
...
calculate_metrics <- function(average_cm_list, cm_list, imputation_type){
  ...
  for (i in 1:length(average_cm_list)){
    ...
    # and where all the metrics are stored by columns (in this order: FDR, Accuracy, MCC)
...
# Calculate metrics once without fold change
...
# Then calculate those metrics with fold change
fold_without_metrics_prot <- calculate_metrics(average_CM_fold_without_prot, confusion_matrices_fold_without_prot, "without")
fold_without_metrics_phos <- calculate_metrics(average_CM_fold_without_phos, confusion_matrices_fold_without_phos, "without")
fold_without_metrics_norm <- calculate_metrics(average_CM_fold_without_norm, confusion_matrices_fold_without_norm, "without")

fold_min_max_metrics_prot <- calculate_metrics(average_CM_fold_min_max_prot, confusion_matrices_fold_min_max_prot, "minmax")
fold_min_max_metrics_phos <- calculate_metrics(average_CM_fold_min_max_phos, confusion_matrices_fold_min_max_phos, "minmax")
fold_min_max_metrics_norm <- calculate_metrics(average_CM_fold_min_max_norm, confusion_matrices_fold_min_max_norm, "minmax")

fold_gaussian_metrics_prot <- calculate_metrics(average_CM_fold_gaussian_prot, confusion_matrices_fold_gaussian_prot, "gaussian")
fold_gaussian_metrics_phos <- calculate_metrics(average_CM_fold_gaussian_phos, confusion_matrices_fold_gaussian_phos, "gaussian")
fold_gaussian_metrics_norm <- calculate_metrics(average_CM_fold_gaussian_norm, confusion_matrices_fold_gaussian_norm, "gaussian")

fold_knn_metrics_prot <- calculate_metrics(average_CM_fold_knn_prot, confusion_matrices_fold_knn_prot, "knn")
fold_knn_metrics_phos <- calculate_metrics(average_CM_fold_knn_phos, confusion_matrices_fold_knn_phos, "knn")
fold_knn_metrics_norm <- calculate_metrics(average_CM_fold_knn_norm, confusion_matrices_fold_knn_norm, "knn")

fold_mle_metrics_prot <- calculate_metrics(average_CM_fold_mle_prot, confusion_matrices_fold_mle_prot, "mle")
fold_mle_metrics_phos <- calculate_metrics(average_CM_fold_mle_phos, confusion_matrices_fold_mle_phos, "mle")
fold_mle_metrics_norm <- calculate_metrics(average_CM_fold_mle_norm, confusion_matrices_fold_mle_norm, "mle")

fold_rf_metrics_prot <- calculate_metrics(average_CM_fold_rf_prot, confusion_matrices_fold_rf_prot, "rf")
fold_rf_metrics_phos <- calculate_metrics(average_CM_fold_rf_phos, confusion_matrices_fold_rf_phos, "rf")
fold_rf_metrics_norm <- calculate_metrics(average_CM_fold_rf_norm, confusion_matrices_fold_rf_norm, "rf")
...
metrics_CM_fold_proteinGroups <- base::rbind(fold_without_metrics_prot, fold_min_max_metrics_prot, fold_gaussian_metrics_prot, fold_knn_metrics_prot, fold_mle_metrics_prot, fold_rf_metrics_prot)
metrics_CM_fold_Phosphosites <- base::rbind(fold_without_metrics_phos, fold_min_max_metrics_phos, fold_gaussian_metrics_phos, fold_knn_metrics_phos, fold_mle_metrics_phos, fold_rf_metrics_phos)
metrics_CM_fold_normedPhosphosites <- base::rbind(fold_without_metrics_norm, fold_min_max_metrics_norm, fold_gaussian_metrics_norm, fold_knn_metrics_norm, fold_mle_metrics_norm, fold_rf_metrics_norm)
...
# Updated this code to make sure that the original matrix has the same rows as both the imputed and the missing matrix
...
calculate_nrmse <- function(imputed_list, missing_list, original_list){
  ...
  for (i in 1:length(imputed_list)){
    ...
    for (j in 1:length(imputed_list[[i]])){
      ...
      # Make sure that the original matrix has the same rows as the imputed and missing ones
      updated_original_matrix <- original_matrix[rownames(original_matrix) %in% rownames(imputed_matrix),]
      
      # Compute its NRMSE
      nrmse_value <- nrmse(imputed_matrix, missing_matrix, updated_original_matrix)
...
# Calculate the NRMSE for all the possible data
nrmse_minmax_prot <- calculate_nrmse(min_max_imputed_matrices_prot, missing_complete_matrices_prot, original_matrices_prot)
nrmse_minmax_phos <- calculate_nrmse(min_max_imputed_matrices_phos, missing_complete_matrices_phos, original_matrices_phos)
nrmse_minmax_norm <- calculate_nrmse(min_max_imputed_matrices_norm, missing_complete_matrices_norm, original_matrices_norm)

nrmse_gaussian_prot <- calculate_nrmse(gaussian_imputed_matrices_prot, missing_complete_matrices_prot, original_matrices_prot)
nrmse_gaussian_phos <- calculate_nrmse(gaussian_imputed_matrices_phos, missing_complete_matrices_phos, original_matrices_phos)
nrmse_gaussian_norm <- calculate_nrmse(gaussian_imputed_matrices_norm, missing_complete_matrices_norm, original_matrices_norm)

nrmse_knn_prot <- calculate_nrmse(knn_imputed_matrices_prot, missing_complete_matrices_prot, original_matrices_prot)
nrmse_knn_phos <- calculate_nrmse(knn_imputed_matrices_phos, missing_complete_matrices_phos, original_matrices_phos)
nrmse_knn_norm <- calculate_nrmse(knn_imputed_matrices_norm, missing_complete_matrices_norm, original_matrices_norm)

nrmse_mle_prot <- calculate_nrmse(mle_imputed_matrices_prot, missing_complete_matrices_prot, original_matrices_prot)
nrmse_mle_phos <- calculate_nrmse(mle_imputed_matrices_phos, missing_complete_matrices_phos, original_matrices_phos)
nrmse_mle_norm <- calculate_nrmse(mle_imputed_matrices_norm, missing_complete_matrices_norm, original_matrices_norm)

nrmse_rf_prot <- calculate_nrmse(rf_imputed_matrices_prot, missing_complete_matrices_prot, original_matrices_prot)
nrmse_rf_phos <- calculate_nrmse(rf_imputed_matrices_phos, missing_complete_matrices_phos, original_matrices_phos)
nrmse_rf_norm <- calculate_nrmse(rf_imputed_matrices_norm, missing_complete_matrices_norm, original_matrices_norm)
...
metrics_fold_overall_proteinGroups <- base::cbind(metrics_CM_fold_proteinGroups, avg_nrmse_prot)
metrics_fold_overall_Phosphosites <- base::cbind(metrics_CM_fold_Phosphosites, avg_nrmse_phos)
metrics_fold_overall_normedPhosphosites <- base::cbind(metrics_CM_fold_normedPhosphosites, avg_nrmse_norm)

# Save those metrics in a txt table format
write.table(metrics_overall_proteinGroups, file="proteinGroups_avg_metrics_more_than_one_complete_value.txt", sep="\t", col.names=NA)
write.table(metrics_overall_Phosphosites, file="Phospho (STY)Sites_avg_metrics_more_than_one_complete_value.txt", sep="\t", col.names=NA)
write.table(metrics_overall_normedPhosphosites, file="Phospho (STY)Sites_normed_avg_metrics_more_than_one_complete_value.txt", sep="\t", col.names=NA)

write.table(metrics_fold_overall_proteinGroups, file="proteinGroups_avg_metrics_with_fold_change.txt", sep="\t", col.names=NA)
write.table(metrics_fold_overall_Phosphosites, file="Phospho (STY)Sites_avg_metrics_with_fold_change.txt", sep="\t", col.names=NA)
write.table(metrics_fold_overall_normedPhosphosites, file="Phospho (STY)Sites_normed_avg_metrics_with_fold_change.txt", sep="\t", col.names=NA)
